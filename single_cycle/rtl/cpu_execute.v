// cpu_execute.v — Layers 1+2+3: Fetch + Decode + Execute
module cpu_execute (
    input  wire clk,
    input  wire rst_n,
    output wire [31:0] pc,
    ...
    // ---------- EXECUTE ----------
    // the alu_src mux: choose immediate or rs2 for the ALU's second input
    wire [31:0] alu_b = alu_src ? imm : rs2_data;

    // simple ALU op decode: for add/addi we want ADD (0); for sub, op 1
    // (full decode comes later; this keeps it simple for now)
